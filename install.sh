...
if [ -d "/opt/SshManagerApi" ]; then
    if verificar_processo "sshmanagerapi"; then
        ...
        if [[ $arch == "x86_64" || $arch == "amd64" || $arch == "x86_64h" ]]; then
            ...
            curl -o "/opt/SshManagerApi/sshmanagerapi" -f "https://raw.githubusercontent.com/UlekBR/SshManagerApiTests/main/sshmanager"
        elif [[ $arch == "aarch64" || $arch == "arm64" || $arch == "armv8-a" ]]; then
            echo "Sistema baseado em arm64 (64-bit ARM)"
            curl -o "/opt/SshManagerApi/sshmanagerapi" -f "https://raw.githubusercontent.com/UlekBR/SshManagerApiTests/main/sshmanagerarm"
        else
        ...
        curl -o "/opt/SshManagerApi/menu.sh" -f "https://raw.githubusercontent.com/UlekBR/SshManagerApiTests/main/menu.sh"
